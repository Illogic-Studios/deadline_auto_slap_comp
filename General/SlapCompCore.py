...
def get_nuke_executable():
    ...
    Returns:
        str: Chemin vers nuke.exe ou fallback C:/Program Files/Nuke15.1v5/Nuke15.1.exe
    """
    try:
                    ...
                    if line.startswith("RenderExecutable"):
                        ...
                        if len(parts) == 2:
                            ...
                            if os.path.exists(nuke_path):
                                ClientUtils.LogText(f"Nuke executable trouve: {nuke_path}")
                                return nuke_path
    except Exception as e:
        ClientUtils.LogText(f"Erreur lecture config Nuke: {str(e)}")

    # Fallback
    fallback_path = r"C:/Program Files/Nuke15.1v5/Nuke15.1.exe"
    ClientUtils.LogText(f"Utilisation fallback Nuke: {fallback_path}")
...
def group_high_prio_and_render_jobs(jobs):
    ...
    for job in jobs:
        ...
        elif clean_name.endswith('_render'):
            clean_name = clean_name[:-len('_render')]
    ...
    if not base_names_set:
        ClientUtils.LogText("  Aucun base_name trouvé avec pattern _v###")
        return {}

    ClientUtils.LogText(f"  Base names trouvés: {len(base_names_set)}")
    for bn in base_names_set:
        ClientUtils.LogText(f"    - {bn}")

    # Étape 2: Pour chaque base_name, chercher TOUTES les versions dans Deadline
    ClientUtils.LogText("  Recherche de TOUTES les versions dans Deadline...")
    all_jobs = RepositoryUtils.GetJobs(True)  # True = include completed
    ...
    for base_name in base_names_set:
        ...
        ClientUtils.LogText(f"    Base '{base_name}': {len(matching_versions)} version(s) trouvée(s)")
        for (bn, ver), jobs_list in sorted(matching_versions.items(), key=lambda x: x[0][1]):
            ClientUtils.LogText(f"      - v{ver:03d} ({len(jobs_list)} job(s))")
...
def get_combined_job_completion(jobs):
    ...
    if total_completed == 0 and total_frames > 0 and all([job.JobStatus == 'Completed' for job in jobs]):
        ClientUtils.LogText(f"  Jobs marked as Completed but 0/{total_frames} frames reported, checking filesystem...")
        ...
        for job in jobs:
            ...
            if output_dirs and len(output_dirs) > 0:
                ...
                ClientUtils.LogText(f"  Checking directory: {output_dir}")
                ...
                if seq_info and seq_info.get('total_frames', 0) > 0:
                    ...
                    ClientUtils.LogText(f"  Filesystem verification: Found {frames_on_disk} frames on disk")
                else:
                    ClientUtils.LogText(f"  Filesystem verification: No frames found in {output_dir}")

    # Calculate percentage
    completion_pct = 0
    if total_frames > 0:
        completion_pct = int((total_completed * 100.0) / total_frames)
        ClientUtils.LogText(f"  Final completion: {total_completed}/{total_frames} frames = {completion_pct}%")
        ...
        try:
            ...
            for subdir in subdirs:
                ...
                if subdir_sequences:
                    ...
                    main_seq = max(subdir_sequences.items(), key=lambda x: len(x[1]['frames']))
            ...
            if all_subfolder_sequences:
                best_subfolder = max(all_subfolder_sequences.items(), key=lambda x: x[1]['frame_count'])
...
def extract_prism_from_filesystem_path(output_dir):
    ...
    Args:
        output_dir (str): Chemin complet (ex: "I:/VCA_Perlee_2510/03_Production/Shots/CAPS02/SH0230/Renders/...")

    Returns:
        dict: {
            'drive': str,              # Ex: "I:"
            'project_root': str,       # Ex: "I:/VCA_Perlee_2510"
            'project': str,            # Ex: "VCA_Perlee_2510"
            'sequence': str,           # Ex: "CAPS02"
            'shot': str,               # Ex: "SH0230"
            'shot_path': str          # Ex: "I:/VCA_Perlee_2510/03_Production/Shots/CAPS02/SH0230"
    ...
    match = re.search(r'([A-Z]:)/([^/]+)/03_Production/Shots/([^/]+)/([^/]+)', normalized, re.IGNORECASE)
...
def extract_project_name(project_root):
    ...
    Args:
        project_root (str): Chemin racine du projet (ex: I:/VCA_Perlee_2510)
...
def scan_prism_render_layers(project_root, sequence, shot):
    ...
    """
    render_base = f"{project_root}/03_Production/Shots/{sequence}/{shot}/Renders/3dRender"

    ClientUtils.LogText(f"    scan_prism_render_layers: Checking path: {render_base}")

    if not os.path.isdir(render_base):
        ClientUtils.LogText(f"    scan_prism_render_layers: Directory NOT FOUND: {render_base}")
        return []

    ClientUtils.LogText(f"    scan_prism_render_layers: Directory exists, listing contents...")
    ...
    try:
        ...
        ClientUtils.LogText(f"    scan_prism_render_layers: Found {len(layer_folders)} items in {render_base}")
    except Exception as e:
        ClientUtils.LogText(f"    scan_prism_render_layers: ERROR listing directory: {str(e)}")
        return []
    ...
    for layer_name in layer_folders:
        ...
        if not os.path.isdir(layer_path):
            ClientUtils.LogText(f"      Skipping '{layer_name}' (not a directory)")
            continue

        ClientUtils.LogText(f"      Processing layer: {layer_name}")

        # Cherche les versions
        version_folders_list = os.listdir(layer_path)
        ClientUtils.LogText(f"        Found {len(version_folders_list)} version folders in {layer_name}")
        ...
        for version_folder in version_folders_list:
            ...
            if not version_match:
                ClientUtils.LogText(f"          Skipping '{version_folder}' (version pattern not matched)")
                continue

            version_num = int(version_match.group(1))
            ClientUtils.LogText(f"          Found version: v{version_num:03d} in folder '{version_folder}'")
            ...
            if seq_info:
                ...
                if 'subfolder' in seq_info:
                    ...
                    ClientUtils.LogText(f"          Images found in subfolder: {seq_info['subfolder']}")
                ...
                ClientUtils.LogText(f"          Added: {layer_name} v{version_num:03d} ({seq_info['total_frames']} frames)")
            else:
                ClientUtils.LogText(f"          Skipping v{version_num:03d} (no image sequence found)")

    ClientUtils.LogText(f"    scan_prism_render_layers: Total layers found: {len(layers)}")
    return layers


def build_prism_slapcomp_paths(shot_path, project, sequence, shot, department, version_number):
    """
...
def extract_prism_from_job_name(job):
    ...
    Example:
        >>> job.JobName = "VCA_Perlee_2510_CAPS02-SH0230_BG_v001_render"
        >>> extract_prism_from_job_name(job)
        {'project': 'VCA_Perlee_2510', 'sequence': 'CAPS02', 'shot': 'SH0230', ...}
    """
    ...
    for pattern, pattern_type in seq_shot_patterns:
        ...
        if match:
            ...
            ClientUtils.LogText(f"  Extracted sequence/shot from job name ({pattern_type}): {sequence}/{shot}")
            break
        ...
        if batch_name:
            for pattern, pattern_type in seq_shot_patterns:
                ...
                if match:
                    sequence = match.group(1).upper()
                    shot = match.group(2).upper()
                    ClientUtils.LogText(f"  Extracted sequence/shot from batch name ({pattern_type}): {sequence}/{shot}")
                    break
    ...
    for pattern in project_patterns:
        ...
        if match:
            ...
            ClientUtils.LogText(f"  Extracted project from job name: {project}")
            break
        ...
        if batch_name:
            for pattern in project_patterns:
                ...
                if match:
                    project = match.group(1).strip('_/\\')
                    project = re.sub(r'[_/\\]+$', '', project)
                    ClientUtils.LogText(f"  Extracted project from batch name: {project}")
                    break

    # Si toujours pas de projet, utiliser un nom par défaut
    if not project:
        project = "UnknownProject"
        ClientUtils.LogText(f"  Could not extract project name, using default: {project}")
...
def get_prism_info_smart(job, output_dirs=None):
    ...
    if output_dirs and len(output_dirs) > 0:
        ...
        if prism_info:
            ClientUtils.LogText("  Prism info extracted from filesystem path")
            return prism_info

    # Stratégie 2: Fallback sur job metadata (robuste mais plus lent)
    prism_info = extract_prism_from_job_name(job)
    if prism_info:
        ClientUtils.LogText("  Prism info extracted from job metadata (fallback)")
...
def find_next_slapcomp_version(scenefile_dir, render_filename):
    ...
    ClientUtils.LogText(f"Scenefile dir: {scenefile_dir}")
    ClientUtils.LogText(f"Render filename base: {render_filename}")

    if not os.path.isdir(scenefile_dir):
        ClientUtils.LogText(f"Directory does not exist, returning version 1")
        return 1

    existing_files = os.listdir(scenefile_dir)
    ClientUtils.LogText(f"Found {len(existing_files)} files in directory")

    max_version = 0
    pattern = re.compile(rf'{re.escape(render_filename)}_v(\d{{3,4}})\.nk$')
    ClientUtils.LogText(f"Pattern to match: {pattern.pattern}")

    for filename in existing_files:
        match = pattern.search(filename)  # Changed from .match() to .search() for robustness
        if match:
            version = int(match.group(1))
            ClientUtils.LogText(f"  Matched file: {filename} -> version {version}")
            max_version = max(max_version, version)
        else:
            ClientUtils.LogText(f"  Skipped file: {filename} (no match)")

    next_version = max_version + 1
    ClientUtils.LogText(f"Max version found: {max_version}, returning next version: {next_version}")
    ClientUtils.LogText(f"=== find_next_slapcomp_version: END ===\n")
...
def get_output_dirs(jobs_to_process, filter_qc_layers=True):
    ...
    ClientUtils.LogText(f"Jobs to process: {len(jobs_to_process)}")

    for idx, job in enumerate(jobs_to_process):
        ClientUtils.LogText(f"  Job {idx + 1}: {job.JobName}")
    ...
    ClientUtils.LogText(f"Retour de group_high_prio_and_render_jobs: {type(grouped_jobs)}")

    ClientUtils.LogText(f"Jobs groupés: {len(grouped_jobs)} groupes (toutes versions)")

    # Log détaillé des groupes pour debug
    for (base_name, version), jobs in grouped_jobs.items():
        ClientUtils.LogText(f"  Groupe: base='{base_name}' v{version:03d} ({len(jobs)} job(s))")
    ...
    for (base_name, version), associated_jobs in grouped_jobs.items():
        ...
        if prism_info:
            context_key = (prism_info['project_root'], prism_info['sequence'], prism_info['shot'])
            prism_contexts[context_key] = prism_info
            ClientUtils.LogText(f"  Contexte ajouté: {prism_info['project']}/{prism_info['sequence']}/{prism_info['shot']}")

    ClientUtils.LogText(f"Total contextes Prism: {len(prism_contexts)}")
    ...
    for (base_name, version), associated_jobs in grouped_jobs.items():
        ...
        if not output_dirs or len(output_dirs) == 0:
            ClientUtils.LogText(f"  Skip job (no output_dir): {job.JobName}")
            continue
        ...
        for pattern, group_idx in layer_patterns:
            ...
            if shot_match:
                ...
                ClientUtils.LogText(f"  Layer extracted via pattern: '{layer_name}' from base: '{base_name}' (pattern #{layer_patterns.index((pattern, group_idx)) + 1})")
                break
        else:
            # Fallback: utilise basename de output_dir
            layer_name = os.path.basename(output_dir)
            ClientUtils.LogText(f"  Layer extracted via fallback (basename): '{layer_name}' from base: '{base_name}'")

        # Détecte la séquence d'images
        seq_info = detect_image_sequence_info(output_dir)

        if not seq_info:
            ClientUtils.LogText(f"  Skip job (no sequence): {output_dir}")
            continue
        ...
        ClientUtils.LogText(f"  Added from Deadline: {layer_name} v{version:03d} ({completion_info['completion']}%)")

    ClientUtils.LogText(f"Layers depuis Deadline: {len(output_info)}")
    ...
    for context_key, prism_info in prism_contexts.items():
        ...
        ClientUtils.LogText(f"  Scanning: {project_root}/{sequence}/{shot}")
        ...
        for fs_layer in filesystem_layers:
            ...
            for existing in output_info:
                if (existing.get('layer_name') == fs_layer['layer_name'] and
                    existing.get('version') == fs_layer['version'] and
                    existing.get('sequence') == sequence and
                    existing.get('shot') == shot):
                    already_exists = True
                    break
            ...
            if not already_exists:
                ...
                filesystem_count += 1
                ClientUtils.LogText(f"    Added from filesystem: {fs_layer['layer_name']} v{fs_layer['version']:03d}")

    ClientUtils.LogText(f"Layers depuis filesystem: {filesystem_count}")

    # Filtre les layers QC si demandé
    if filter_qc_layers:
        before_filter = len(output_info)
        output_info = [info for info in output_info if not info['layer_name'].upper().startswith('QC')]
...
def group_output_info_for_ui(output_info):
    ...
    ClientUtils.LogText(f"Found {len(layers_dict)} unique layers")
    ...
    for layer_name, entries in layers_dict.items():
        ...
        for entry in entries_sorted:
            ...
            frames_completed = int((completion * total_frames) / 100.0) if total_frames > 0 else 0
        ...
        grouped_entry = {
            ...
            'selected_version_index': len(versions_list) - 1,  # Dernière version par défaut
        ...
        ClientUtils.LogText(f"  Layer '{layer_name}': {len(versions_list)} version(s), source={source}")

    ClientUtils.LogText(f"=== group_output_info_for_ui: END - {len(grouped_output)} layers ===\n")
...
def render_nuke_script(nuke_script_path, first_frame, last_frame):
    ...
    ClientUtils.LogText(f"  Commande: {' '.join(cmd)}")
    ...
    try:
        ...
        for line in iter(process.stdout.readline, ''):
            if line:
                ClientUtils.LogText(line.rstrip())
        ...
        ClientUtils.LogText(f"Erreur lancement rendu: {str(e)}")


def submit_to_deadline(nuke_script_path, first_frame, last_frame, output_info, render_dir, render_filename, job_name=None):
    """
    ...
    submission_path = os.path.join(repo_path, "custom", "scripts", "Submission", "SlapComp")
    ...
    try:
        ...
        ClientUtils.LogText(f"Fichier Nuke: {normalize_path_for_nuke(nuke_script_path)}")
        ClientUtils.LogText(f"Frame range: {first_frame}-{last_frame}")
        ClientUtils.LogText(f"Dépendances: {len(dependency_job_ids)} job(s)")
        for jid in dependency_job_ids:
            ClientUtils.LogText(f"  - {jid}")

        if render_dir and render_filename:
            ClientUtils.LogText(f"Render dir: {normalize_path_for_nuke(render_dir)}")
            ClientUtils.LogText(f"Render filename: {render_filename}")
        ...
        if job_id:
            ClientUtils.LogText(f"\n=== SUCCESS ===")
            ClientUtils.LogText(f"Job soumis: {job_id}")
        else:
            ClientUtils.LogText(f"\n=== ERREUR ===")
            ClientUtils.LogText(f"Échec de la soumission")

    except Exception as e:
        ClientUtils.LogText(f"ERREUR: {str(e)}")
        import traceback
        traceback.print_exc()
...
def call_nuke_script(output_info, render_mode="none"):
    ...
    if not output_info or len(output_info) == 0:
        ClientUtils.LogText("Aucune donnée à traiter")
        return
    ...
    if prism_info:
        ...
        shot_path = prism_info['shot_path']
        project = prism_info.get('project', project)  # Override si trouvé dans prism_info
        sequence = prism_info['sequence']
        shot = prism_info['shot']
        ...
        temp_paths = build_prism_slapcomp_paths(shot_path, project, sequence, shot, department, 1)
        ...
        paths = build_prism_slapcomp_paths(shot_path, project, sequence, shot, department, next_version)
        ...
        ClientUtils.LogText(f"Projet: {project}")
        ClientUtils.LogText(f"Séquence: {sequence}")
        ClientUtils.LogText(f"Shot: {shot}")
        ClientUtils.LogText(f"Département: {department}")
        ClientUtils.LogText(f"Version: v{next_version:03d}")
        ClientUtils.LogText(f"Job name: {job_name}")
    else:
        ...
        next_version = find_next_slapcomp_version(scenefile_dir, render_filename_base)
        output_nk = os.path.join(scenefile_dir, f"{render_filename_base}_v{next_version:03d}.nk")
    ...
    ClientUtils.LogText(f"  Scenefile: {normalize_path_for_nuke(output_nk)}")
    ClientUtils.LogText(f"  Render dir: {normalize_path_for_nuke(render_dir)}")
    ClientUtils.LogText(f"  Render filename: {render_filename}")
...
root.knob("customOCIOConfigPath").setValue("R:/pipeline/networkInstall/OpenColorIO-Configs/PRISM/illogic_V01-cg-config-v1.0.0_aces-v1.3_ocio-v2.1.ocio")
    ...
    try:
        ...
        if process.returncode == 0:
            if os.path.exists(output_nk):
                ClientUtils.LogText(f"\n=== SUCCESS ===")
                ClientUtils.LogText(f"Slap comp cree: {normalize_path_for_nuke(output_nk)}")
                ...
                if render_mode != "none":
                    ...
                    if render_mode == "local":
                        ...
                        submit_to_deadline(output_nk, all_first, all_last, output_info, render_dir, render_filename, job_name)
            else:
                ClientUtils.LogText(f"\n=== ATTENTION ===")
                ClientUtils.LogText(f"Le fichier n'existe pas: {normalize_path_for_nuke(output_nk)}")
        else:
            ClientUtils.LogText(f"\n=== ERREUR ===")
            ClientUtils.LogText(f"Code retour: {process.returncode}")
...
def load_preset(project, sequence, shot):
    ...
    if not preset_dir or not os.path.exists(preset_dir):
        ClientUtils.LogText("Aucun répertoire de presets trouvé")
        return None
    ...
    if shot:
        ...
        if os.path.exists(shot_overrides_path):
            try:
                ...
                ClientUtils.LogText(f"Erreur lecture shot_overrides.json: {str(e)}")
    ...
    if not preset_data:
        ...
        if os.path.exists(project_preset_path):
            try:
                ...
                ClientUtils.LogText(f"Erreur lecture project_preset.json: {str(e)}")

    if preset_data:
        ClientUtils.LogText(f"Preset chargé: {preset_source}")
    else:
        ClientUtils.LogText("Aucun preset trouvé - utilisation ordre par défaut")
...
def apply_preset_data(output_info, preset_data):
    ...
    for layer_name, info in layers_dict.items():
        if layer_name not in layer_order:
            info['included'] = False
            reordered.append(info)
            ClientUtils.LogText(f"Layer '{layer_name}' non présent dans preset, ajouté à la fin (non inclus)")
...
def save_preset_project(project, preset_data):
    ...
    if not preset_dir:
        ClientUtils.LogText("Erreur: impossible de déterminer le répertoire de presets")
        return None
    ...
    if not os.path.exists(preset_dir):
        try:
            ...
            ClientUtils.LogText(f"Dossier créé: {preset_dir}")
        except Exception as e:
            ClientUtils.LogText(f"Erreur création dossier: {str(e)}")
            return None
    ...
    try:
        ...
        ClientUtils.LogText(f"Preset projet sauvegardé: {project_preset_path}")
...
def save_preset_shot(project, sequence, shot, preset_data):
    ...
    try:
        ...
        ClientUtils.LogText(f"Preset shot sauvegardé: {shot_key}")
        return shot_key
    except Exception as e:
        ClientUtils.LogText(f"Erreur sauvegarde shot override: {str(e)}")
        return None
...
def select_latest_complete_versions(output_info):
    ...
    for layer_key, versions in layers_dict.items():
        ...
        if complete_versions:
            ...
            ClientUtils.LogText(f"  {latest.get('layer_name')}: v{latest.get('version'):03d} (100%)")
        else:
            ...
            ClientUtils.LogText(f"  {latest.get('layer_name')}: v{latest.get('version'):03d} ({completion}%) - ATTENTION: incomplet!")
