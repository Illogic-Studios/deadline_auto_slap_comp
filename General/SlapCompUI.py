...
def get_layer_name(info):
    ...
    if "versions" in info and "selected_version_index" in info:
        ...
        return (
            f"{source_icon} {layer_name} - {version} ({completion:.0f}%) {status_icon}"
        )
    else:
...
def on_move_up(*args):
    ...
    try:
        ...
        layer_names = [
            get_layer_name(output_info_global[idx]) for idx in included_indices
        ]
        ...
        output_info_global[real_index], output_info_global[real_prev_index] = (
            output_info_global[real_prev_index],
            output_info_global[real_index],
        )

        refresh_ui_controls()
        refresh_list()
        scriptDialog.SetValue(
            "LayerList", get_layer_name(output_info_global[real_prev_index])
        )
        update_preview()
...
def on_move_down(*args):
    ...
    try:
        ...
        output_info_global[real_index], output_info_global[real_next_index] = (
            output_info_global[real_next_index],
            output_info_global[real_index],
        )
        ...
        refresh_list()
        scriptDialog.SetValue(
            "LayerList", get_layer_name(output_info_global[real_next_index])
        )
        update_preview()
    except Exception as e:
    ...
    try:
        ...
        for v_idx, v in enumerate(info["versions"]):
            ...
            if label == selected_label:
                ...
                ClientUtils.LogText(
                    f"Status mis a jour pour layer {idx}: {v['status']} ({completion:.0f}%)"
                )
                break
    except Exception as e:
...
def get_included_indices():
    ...
    return [
        idx
        for idx in range(len(output_info_global))
...
def update_preview():
    ...
    if len(included_indices) == 0:
        ...
        preview_lines.append(
            f"Read[0]: {get_layer_name(output_info_global[first_idx])}"
        )
        ...
        for i in range(1, len(included_indices)):
            ...
            if i == 1:
                ...
                preview_lines.append(
                    f"Merge{i} [{merge_op}] (A=Merge{i - 1}, B=Read[{i}])"
                )
...
def show_slap_comp_dialog(output_info):
    ...
    scriptDialog.AddControlToGrid(
        "InfoLabel",
        "LabelControl",
        "Configurez l'ordre et les versions des layers:",
        0,
        0,
        colSpan=6,
    )
    ...
    # Headers
    scriptDialog.AddControlToGrid(
        "HeaderInclude", "LabelControl", "Include", 2, 0, expand=False
    )
    scriptDialog.AddControlToGrid(
        "HeaderSource", "LabelControl", "Source", 2, 1, expand=False
    )
    scriptDialog.AddControlToGrid(
        "HeaderLayer", "LabelControl", "Layer", 2, 2, expand=False
    )
    scriptDialog.AddControlToGrid(
        "HeaderVersion", "LabelControl", "Version", 2, 3, expand=False
    )
    scriptDialog.AddControlToGrid(
        "HeaderStatus", "LabelControl", "Status", 2, 4, expand=False
    )
    scriptDialog.AddControlToGrid(
        "HeaderMergeOp", "LabelControl", "Merge Op", 2, 5, expand=False
    )
    ...
    for idx, info in enumerate(output_info_global):
        ...
        includeCheck = scriptDialog.AddControlToGrid(
            f"IncludeCheck{idx}", "CheckBoxControl", "", row, 0, expand=False
        )
        scriptDialog.SetValue(f"IncludeCheck{idx}", included)
        ...
        source_label = "Deadline" if source == "deadline" else "Filesystem"
        scriptDialog.AddControlToGrid(
            f"SourceLabel{idx}", "LabelControl", source_label, row, 1, expand=False
        )

        # Nom du layer
        scriptDialog.AddControlToGrid(
            f"LayerName{idx}", "LabelControl", layer_name, row, 2, expand=False
        )
        ...
        if "versions" in info:
            ...
            versionCombo.ValueModified.connect(
                lambda _idx=idx: on_version_changed(_idx)
            )
            ...
            status_text = f"<font color='{color}'>{selected_version['status']} ({selected_version['frames_completed']}/{selected_version['frames_total']} frames)</font>"
            scriptDialog.AddControlToGrid(
                f"StatusLabel{idx}", "LabelControl", status_text, row, 4
            )
        ...
        mergeOpCombo = scriptDialog.AddComboControlToGrid(
            f"MergeOpCombo{idx}", "ComboControl", default_merge_op, merge_ops, row, 5
        )
        # Utilise une lambda qui ignore les arguments du signal et passe uniquement idx
        mergeOpCombo.ValueModified.connect(
            lambda *args, _idx=idx: on_merge_op_changed(_idx)
        )

        row += 1

    scriptDialog.AddControlToGrid(
        "Separator2", "SeparatorControl", "", row, 0, colSpan=6
    )
    row += 1

    # Boutons de réordonnement
    scriptDialog.AddControlToGrid(
        "InfoReorder", "LabelControl", "Reordonnancement:", row, 0, colSpan=6
    )
    row += 1
    ...
    moveUpButton = scriptDialog.AddControlToGrid(
        "MoveUpButton", "ButtonControl", "Move Up", row, 0, expand=False
    )
    moveUpButton.ValueModified.connect(on_move_up)

    moveDownButton = scriptDialog.AddControlToGrid(
        "MoveDownButton", "ButtonControl", "Move Down", row, 1, expand=False
    )
    moveDownButton.ValueModified.connect(on_move_down)
    row += 1

    scriptDialog.AddControlToGrid(
        "Separator3", "SeparatorControl", "", row, 0, colSpan=6
    )
    row += 1

    # Preview
    scriptDialog.AddControlToGrid(
        "PreviewLabel", "LabelControl", "Preview:", row, 0, colSpan=6
    )
    row += 1
    scriptDialog.AddControlToGrid(
        "PreviewText", "MultiLineTextControl", "", row, 0, colSpan=6
    )
    row += 1

    scriptDialog.AddControlToGrid(
        "Separator4", "SeparatorControl", "", row, 0, colSpan=6
    )
    row += 1
    ...
    scriptDialog.AddControlToGrid(
        "RenderModeLabel", "LabelControl", "Mode de rendu:", row, 0, expand=False
    )
    ...
    scriptDialog.AddControlToGrid(
        "Separator5", "SeparatorControl", "", row, 0, colSpan=6
    )
    row += 1

    # Boutons OK/Cancel
    okButton = scriptDialog.AddControlToGrid(
        "OkButton", "ButtonControl", "OK", row, 0, expand=False
    )
    okButton.ValueModified.connect(on_ok_clicked)

    cancelButton = scriptDialog.AddControlToGrid(
        "CancelButton", "ButtonControl", "Cancel", row, 1, expand=False
    )
                ...
                result.append(
                    {
                        ...
                        "job_ids": selected_version.get(
                            "job_ids", [selected_version.get("job_id")]
